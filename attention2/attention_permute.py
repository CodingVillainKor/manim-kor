...
class AttentionPermutation(MovingCameraScene):
    def construct(self):
        self.sceneA()
    ...
    def sceneA(self):
        ...
        group = [keys, values]
        hidden_group = [multiples, l_text, w_text]
        permutation = [3, 4, 5, 0, 1, 2]
        self.key_permutation(group, hidden_group, permutation)
    ...
    def key_permutation(self, group, hidden_group, permutation):
        positions = []
        for i, p in enumerate(permutation):
            for row in group:
                row[i].generate_target()
                row[i].target.move_to(row[p])
            positions.append([row[i].get_center() for row in hidden_group])
            for j, row in enumerate(hidden_group):
                if p >= i:
                    row[i].move_to(row[p])
                else:
                    row[i].move_to(positions[p][j])
