...
class PositionalEncodingPermutation(MovingCameraScene):
    def construct(self):
        self.sceneA()
    ...
    def sceneA(self):
        ...
        keys_permuted = VGroup(*[keys[i] for i in permutation])
        pes = VGroup(
        ...
        self.key_permutation([keys], permutation)
        ...
        self.playw(LaggedStart(*[FadeOut(pes[i], target_position=keys_permuted[i]) for i in range(len(pes))], lag_ratio=0.3))
        mixed_color = [self.weighted_sum_hex([keys_permuted[i].get_color().hex, pes[i].get_color().hex], [0.5, 0.5]) for i in range(len(pes))]

        self.playw(LaggedStart(*[keys_permuted[i].animate.set_color(mixed_color[i]).set_stroke_color(WHITE) for i in range(len(keys_permuted))]))
        # TODO: keys 순서 바꿔서 Positional encoding 더하는거 짜야함, 새 클래스에서

    def key_permutation(self, group, permutation):
        for i, p in enumerate(permutation):
            for row in group:
                row[i].generate_target()
                row[i].target.move_to(row[p])
        self.playw(*[MoveToTarget(item) for row in group for item in row])
